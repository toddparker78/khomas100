...
export default function RegisterPage() {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      clubTeam: (form.clubTeam as HTMLInputElement).value,
      emergencyName: (form.emergencyName as HTMLInputElement).value,
      emergencyPhone: (form.emergencyPhone as HTMLInputElement).value,
      consent: (form.consent as HTMLInputElement).checked,
    };
    ...
    try {
      ...
      if (res.ok) {
        setOk("Entry submitted successfully!");
        form.reset();
      } else {
        setOk("Error submitting entry: " + result.error);
      }
    } catch (err) {
      console.error(err);
      setOk("Unexpected error submitting entry.");
    } finally {
      setPending(false);
    }
  };
